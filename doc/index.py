...
class FileIndex:
    ...
    def __init__(self, sandbox_root: Path, db_path: Optional[Path] = None) -> None:
        ...
        self.db_path = db_path or (Path.home() / ".echolocate" / "file_index.sqlite3")
...
def ensure_built(sandbox_root: Path, background_refresh: bool = True) -> FileIndex:
    ...
    if start_build:
        def _run_initial_build():
            ...
            try:
                from echolocate.mcp_server.usn_catchup import catch_up
                print(f"[FileIndex] Checking for a fast catch-up for {idx.sandbox_root}...")
                caught_up = catch_up(idx, idx.sandbox_root, verbose=True)
            except ImportError:
                pass  # pywin32 or usn_catchup unavailable -- full build below
            except Exception as exc:
                print(f"[FileIndex] catch-up attempt failed unexpectedly ({exc}) -- "
                      f"falling back to a full build.")

            if not caught_up:
                print(f"[FileIndex] Indexing {idx.sandbox_root} in the background "
                      f"(results fill in progressively — no need to wait for this to finish)...")
                try:
                    n = idx.build(progress_cb=lambda c: None)
                    print(f"[FileIndex] Indexed {n} files.")
                    idx._record_journal_cursor_best_effort()
                except Exception as exc:
                    print(f"[FileIndex] initial build failed: {exc}")
            ...
            if background_refresh and not idx._monitoring_started:
                ...
                if watching:
                    print(f"[FileIndex] Real-time monitoring active for {idx.sandbox_root}.")
                else:
                    print(f"[FileIndex] Polling every 5 minutes for {idx.sandbox_root} "
                          f"(real-time watcher unavailable).")
